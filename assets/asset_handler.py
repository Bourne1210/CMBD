...
def log(log_type,msg=None,asset=None,new_asset=None,request=None):
	...
	elif log_type == "approved_faild":
		event.name = "{}<{}>: 审批失败".format(new_asset.asset_type,new_asset.sn)
...
class ApproveAsset:
	...
	def _server_upline(self):
		# 在实际的生产环境中，下面的操作应该是原子性的整体事物，任何一步出现异常，所有操作都要回滚。
	...
	def _create_asset(self):
		...
		asset = models.Asset.objects.create(asset_type=self.new_asset.asset_type,
											name = "{}: {}".format(self.new_asset.asset_type,self.new_asset.sn),
	...
	def _create_CPU(self,asset):
		...
		创建CPU
		对发送过来的数据采取了最大限度的容忍，
		实际情况下，可能还要对数据的完整性、合法性、数据类型进行检测，
		根据不同检测情况，是被动接受，还是打回去要求重新收集，请自行决定。
		这里的业务逻辑非常复杂，不可能面面俱到。
		:param asset:
		:return:
		"""
	...
	def _create_RAM(self,asset):
		...
		for ram_dict in ram_list:
			if not ram_dict.get('slot'):
				raise ValueError("未知的内存插槽！") #使用虚拟机时可能无法获取内存插槽，需要你修改此处的逻辑
			ram = models.RAM()
			ram.asset = asset
	...
	def _creat_disk(self,asset):
		"""
		存储设备种类多，还有Raid情况，需要根据实际情况具体解决。
		这里只以简单的SATA硬盘为例子。可能有多快硬盘。
		:param asset:
		:return:
		"""
		...
		for disk_dict in disk_list:
			if not disk_dict.get('sn'):
				raise ValueError('未知的sn的硬盘！') # 根据sn确定具体某块硬盘
			disk = models.Disk()
			disk.asset = asset
	...
	def _delete_original_asset(self):
		...
		也可以设置为修改成已经审批状态但是不删除，只是在管理界面特别处理，不让再次审批，灰色显示
		不过这样可能导致待审批区越来越大
		:return:
		"""
...
class UpdateAsset:
	...
	def asset_update(self):
		# 为以后的其他类型资产扩展留下接口
		func = getattr(self,"_{}_update".format(self.report_data['asset_type']))
		ret = func()
		return ret
	...
	def _update_RAM(self):
		...
		使用集合类型中差的概念，处理不同的情况。
		如果新数据有，但原数据没有，则新增。
		如果新数据没有，原来的数据有，则删除原来多余的部分 。
		如果新的和原数据都有，则更新。
		在原则上下面的代码应该写成一个服用函数，
		但是内存、硬盘、网卡在某些方面的差别，导致很难提取出重用的代码。
		:return:
		"""
		# 获取已经有内存信息，并转成字典格式
		...
		if new_rams_dict:
			for key in new_rams_dict:
				defaults = {
					...
					'capacity': new_rams_dict[key].get('capacity'),
				}
	...
	def _update_nic(self):
		...
		# 需要删除的
		need_deleted_keys = set(old_nics_dict.keys) - set(new_nic_dict.keys())
		if need_deleted_keys:
			for key in need_deleted_keys:
