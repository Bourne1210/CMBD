...
class Win32Info(object):
	...
	def collect(self):
		...
		data.update(self.get_metherboard_info())
	...
	def get_cpu_info(self):
		...
		cpu_model = cpu_lists[0].Name #CPU型号（所有cpu型号都是一样的）
	...
	def get_ram_info(self):
		...
		data = []
		#这个模块是用sql语言获取数据
		ram_collectins = self.wmi_service_connector.ExecQuery("Select * from Win32_PhysicalMemory")
		for ram in ram_collectins: #主机中存在很多内存，要循环所有的内存数据
	...
	def get_disk_info(self):
		...
		"""
		data = []
		for disk in self.wmi_obj.Win32_DiskDrive():  #每块硬盘都要获取相应的信息
			disk_data = {}
	...
	def get_nic_info(self):
		...
		"""
		data = []
		# print(self.wmi_obj.Win32_NetworkAdapterConfiguration())
		for nic in self.wmi_obj.Win32_NetworkAdapterConfiguration():
			# print(nic)
			if nic.MACAdress is not None:
				nic_data = {}
				nic_data["mac"] = nic.MACAddress
				nic_data["model"] = nic.Caption
				nic_data["name"] = nic.Index
				if nic.IPAdress is not None:
					nic_data["ip_address"] = nic.IPAdress[0]
